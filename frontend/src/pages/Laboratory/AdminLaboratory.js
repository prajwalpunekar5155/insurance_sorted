...
function Laboratory() {
    ...
    const getAppointmentList = async () => {
      ...
      if (subadmin_key) {
        ...
        const res = await fetch(
          // `http://localhost:8085/getLaboratories?token_key=${subadmin_key}`
          `http://localhost:8085/getAllLaboratories`
        );
  ...
  const handleDownloadExcel = () => {
    window.open("http://localhost:8085/downloadLaboratories", "_blank");
  };
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(
        `http://localhost:8085/deleteLaboratory/${deleteId}`,
        {
          method: "DELETE",
        }
      );
