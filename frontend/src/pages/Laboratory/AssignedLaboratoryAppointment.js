...
function Appointment() {
  ...
  useEffect(() => {
    const getAppointmentList = async () => {
      ...
      if (!lab_id || !status) return; // Ensure both values are present

      try {
        const res = await fetch(
          `http://localhost:8085/getLaboratoryAppointmentByStatus/${status}?lab_id=${lab_id}`
        );
    ...
    getAppointmentList();
  }, [status]); // Only status is a dependency since lab_id is taken from session
