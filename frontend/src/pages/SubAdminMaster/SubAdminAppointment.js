...
function Appointment() {
  ...
  const [videoAppointmentDetails, setVideoAppointmentDetails] = useState(false);
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState(""); // Track the search query
  const itemsPerPage = 50; // Number of items per page
  const [laboratories, setLaboratories] = useState([]); // Store laboratory list
    ...
    const getAppointmentList = async () => {
      ...
      try {
        const res = await fetch(
          `http://localhost:8085/gettotalappointmentforSubadmin?subadmin_id=${subadmin_id}`,
          {
            method: "GET",
            headers: {
  ...
  useEffect(() => {
    const fetchLaboratories = async () => {
      ...
      if (token_key) {
        ...
        const res = await fetch(
          `http://localhost:8085/getLaboratories?token_key=${token_key}`
        );

        if (res.ok) {
          const getData = await res.json();
          setLaboratories(getData);
  ...
  const handleLabChange = async (event) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8085/getallfilteredappointmentforLaboratory?lab_id=${labId}`
      );
      ...
      setFilteredAppointments([]); // Fallback to empty array
    }
  };
  ...
  const handleVideoClick = () => {
    setVideoAppointmentDetails(true);
  };

  const handleVideoCloseModal = () => {
    setVideoAppointmentDetails(false);
  };
                  ...
                  <div className="row g-4">
                    ...
                    <div className="col-md-6">
                      ...
                      >
                        ...
                        {selectedAppointment.images ? (
                          JSON.parse(selectedAppointment.images).map(
                            (image, index) => (
                              <img
                                key={index}
                                src={`http://localhost:8085/${image}`}
                                ...
                                onClick={() => handleImageClick(image)} // Open preview modal
                              />
                            )
                          )
                        ) : (
                    ...
                    <div className="col-md-6">
                      ...
                      >
                        ...
                        {selectedAppointment.video ? (
                          <video
                            src={`http://localhost:8085/${selectedAppointment.video}`}
                            style={{
                              width: "100px",
                              height: "100px",
                              objectFit: "cover",
                            ...
                            controls // Allow play/pause
                            onClick={handleVideoClick} // Opens the preview modal
                          />
                        ) : (
                    ...
                    >
                      ...
                      <DialogActions>
                        <Button onClick={handleVideoCloseModal} color="primary">
                          Close
                        </Button>
                      </DialogActions>
                    </Dialog>
