...
function Appointment() {
  ...
  useEffect(() => {
    const getAppointmentList = async () => {
      ...
      if (!subadmin_id || !status) return; // Ensure both values are present

      try {
        const res = await fetch(
          `http://localhost:8085/getSubadminAppointmentByStatus/${status}?subadmin_id=${subadmin_id}`
        );
    ...
    getAppointmentList();
  }, [status]); // Only status is a dependency since subadmin_id is taken from session
